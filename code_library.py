queries = [
"""
def standardize_dates(df, col): This function standardizes the format of date values in a specified column (col) of a DataFrame (df) to the format 'YYYY-MM-DD'. It converts date strings to datetime format.
""",
"""
def clean_numbers(df, col): This function cleans a column (col) containing number values in a DataFrame (df). It removes non-numeric characters and converts the values to float. It also replaces the decimal symbol with a period if specified.
""",
"""
def delete_column(df, col): This function deletes a specified column (column_name) from a DataFrame (df) if it exists. It uses the drop() method of pandas DataFrame.
""",
"""
def delete_rows(df, col, condition): This function deletes rows from a DataFrame (df) based on a condition (condition) in a specified target column (target_col). It uses the drop() method of pandas DataFrame.
""",
"""
def fill_nan_and_blanks(df, col): This function replaces missing values (NaN) and blanks in a specified column of a DataFrame (df) with a predefined replacement. It utilizes the fillna() method of pandas DataFrame.
""",
"""
def correct_misspelling(df, col): This function corrects misspelled words in a specified column (col) of a DataFrame (df). It identifies possible misspellings, checks if the column contains only strings, and returns a DataFrame with corrected misspellings.
""",
"""
def compare_and_clean(df, target_col, ref_col): This function compares a target column (target_col) with a reference column (ref_col), and cleans the target column based on the reference. It aims to compare the values in the target column with those in the reference column and clean the target column by replacing its values with the most frequent value corresponding to the same value in the reference column.
""",
"""
def translate_between_columns(df, target_col, ref_col): This function translates the values in the target column to the reference column and vice versa, whichever is null. It uses the googletrans library to translate between english and russian.
""",
"""
def remove_corp_keywords(df, col, keyword_file, new_col): This function reads in a list of corporate keywords from a file (keyword_file), then removes the corporate keywords from the column and creates a new column with the removed keywords.
""",
"""
def lookup_table_and_map_to_new_column(df, col, lookup_file, ref_col, target_col, new_col): This function takes a reference document (lookup_file) to lookup. It then maps the values in the column (col) of the dataframe (df) to the reference document (ref_col), then df creates a new column (new_col) in the dataframe with the mapped values (target_col). 
""",
"""
def clean_strings(df, col): This function clean the strings in a column (col) by removing all punctuation marks, quotation marks, and extra space characters for each value, then returns the dataframe (df).
"""
]
functions = [
"""
...
"""
	def correct_misspelling(df, col):
        ...
        try:
            ...
            word2vec_model.save('./support_docs/en_model.kv')
            word2vec_model = KeyedVectors.load('./support_docs/en_model.kv', mmap='r')
